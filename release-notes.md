...
- 调整：macOS ad-hoc 签名关闭 Hardened Runtime，用于验证安装后启动卡住的问题。
- 安装说明：仍未使用 Developer ID 签名或 Apple 公证；此项调整不代表已获得系统信任。
- 已知问题：本机测试中，临时目录可正常启动，但安装到 Applications 后仍可能卡住。本版本用于进一步验证，不代表该问题已修复。
